...
(function() {
  ...
  var markerElements = document.querySelectorAll('#mapcontainer .mapmarker .markerscene');
  ...
  scenes.forEach(function(scene) {
    ...
    if(scene.data.id == '0-100'){
      currentScene = scene;
    }

    var dataMarker = document.querySelector('#mapcontainer .mapmarker .markerscene[data-id="' + scene.data.id + '"]');
    dataMarker.addEventListener('click', function() {
      switchScene(scene);
      currentScene = scene;
 ...
 // Zoom on tap.
  ...
  function addMarker(e) {
    ...
    currentScene.scene.hotspotContainer().createHotspot(el, markerCoordinates);
  ...
  function updateSceneList(scene) {
    ...
    for(var i=0; i< markerElements.length; i++){
        var markerIcon = markerElements[i];
        if(markerIcon.getAttribute('data-id')==scene.data.id){
            markerIcon.classList.add('current-marker');
            markerIcon.setAttribute('src','images/_ggMapPin.png');
        }else{
          markerIcon.classList.remove('current-marker');
          markerIcon.setAttribute('src','images/_ggMapPinBlue.png');
        }

        if(scene.data.id == '2-102'){
          $("div.topaskin").scrollLeft(70);
        }
        if(scene.data.id == '3-103'){
          $("div.topaskin").scrollLeft(150);
        }
        if(scene.data.id == '4-104'){
          $("div.topaskin").scrollLeft(200);
        }
        if(scene.data.id == '6-106'){
          $("div.topaskin").scrollLeft(250);
        }
        if(scene.data.id == '7-107'){
          $("div.topaskin").scrollLeft(350);
        }

        if(scene.data.id == '8-108'){
          $("div.topaskin").scrollLeft(400);
        }
        if(scene.data.id == '9-109'){
          $("div.topaskin").scrollLeft(450);
        }
        if(scene.data.id == '10-110'){
          $("div.topaskin").scrollLeft(500);
        }
        if(scene.data.id == '11-111'){
          $("div.topaskin").scrollLeft(550);
        }
        if(scene.data.id == '12-112'){
          $("div.topaskin").scrollLeft(600);
        }
    }
  }
  ...
  function createLinkHotspotElement(hotspot) {
    ...
    wrapper.addEventListener('click', function() {
      ...
      currentScene = findSceneById(hotspot.target);
    });
